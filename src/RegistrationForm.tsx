...
export default function RegistrationForm({ 
  ...
  const buildPayload = (data: typeof formData) => {
    ...
    const appendField = (key: string, value: any) => {
      // Don't skip fields that might be required by the backend, even if empty
      // Only skip if key is empty
      if (!key) return;
    ...
    // ATTACHMENTS - The backend expects these as an array under the 'document' key
    if (data.birthCertificate) {
      payload.append("document[]", data.birthCertificate);
      // Also append individually if required by some endpoints
      payload.append("birth_certificate", data.birthCertificate);
    }
    if (data.barangayCertificate) {
      payload.append("document[]", data.barangayCertificate);
      // Also append individually if required by some endpoints
      payload.append("barangay_certificate", data.barangayCertificate);
    }

    if (data.idPhoto) {
      payload.append("id_photo", data.idPhoto);
    }
  ...
  const handleSubmit = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      // Using axios for more reliable FormData submission
      const response = await axios.post("/api/proxy/dbosca/applications", payload, {
        headers
      });
      ...
      if (err.response) {
        ...
        errorMessage = data?.message || `Error: ${err.response.status}`;
        if (data?.errors) {
