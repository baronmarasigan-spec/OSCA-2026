...
async function startServer() {
  ...
  // API Proxy - Handle this BEFORE body parsers for multipart/form-data support
  app.use(
    "/api/proxy/dbosca",
    createProxyMiddleware({
      target: "https://api-dbosca.drchiocms.com/api",
      changeOrigin: true,
      pathRewrite: { "^/api/proxy/dbosca": "" },
      onProxyRes: (proxyRes, req, res) => {
        // Log proxy responses for debugging if needed
        if (proxyRes.statusCode && proxyRes.statusCode >= 400) {
          console.log(`[PROXY ERROR] ${req.method} ${req.url} -> ${proxyRes.statusCode}`);
        }
      },
      onError: (err, req, res) => {
  ...
  // LCR Proxy
  app.get("/api/proxy/lcr", async (req, res) => {
    const query = new URLSearchParams(req.query as any).toString();
    const url = `https://lcrdev.pylontradingintl.com/api/client/record/list/birth${query ? '?' + query : ''}`;
    try {
      ...
      try {
        ...
        console.warn("Proxy: Failed to parse response as JSON", text);
        data = { message: text };
      }
      res.status(response.status).json(data);
    } catch (error) {
      console.error("Proxy error:", error);
      res.status(500).json({ error: "Failed to fetch from LCR API" });
    }
  });
